...
writeStatus = ->
	string = ''
	string += LOG.sass+'|'
	string += LOG.coffeescript+'|'
	string += LOG.html+'|'
	string += LOG.javascript+'|'
	string += LOG.css+'|'
	string += LOG.ihtml
	fs.writeFile 'dev-tools/status.txt', string
...
gulp.task 'CoffeeScript', ->
	...
	writeLog log, 'file'
	LOG.coffeescript = 'Writed'
	log
		.pipe plumber
			errorHandler: (err)->
				_CoffeeConsole err
				LOG.coffeescript = 'ERROR :: '+err.stack
				writeStatus()
		.pipe do fileinclude
		.pipe do coffee
